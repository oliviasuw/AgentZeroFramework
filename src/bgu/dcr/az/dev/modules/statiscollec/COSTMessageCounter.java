...
public class COSTMessageCounter extends AbstractStatisticCollector<COSTMessageCounter.COSTMsgRecord> {
    ...
    @Override
    public void submit(COSTMsgRecord record) {
    ...
    public VisualModel analyze(Database db, Test r) {
    	try {
            ...
            switch (graphType) {
            case BY_AGENT:
                bv = new BarVisualModel("COST Messages Count", "Agent", "Avg(Message Sent)");
                res = db.query(""
            ...
            case BY_TESTFILE:
                ...
                bv = new BarVisualModel("COST Messages Count", runVar, "Sum(Message Sent)");
                res = db.query(""
            ...
            Logger.getLogger(COSTMessageCounter.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//                final VarAgentMap varAgentMap = new VarAgentMap(files[fileNo-1]);
        ...
        new Hooks.TerminationHook() {
            ...
            public void hook() {
                for (int i = 0; i < counts.length; i++) {
                    submit(new COSTMsgRecord(i, counts[i], testFile));
                }
            }
        }.hookInto(ex);
    }
    ...
    public String getName() {
    	return "COST Messages Count";
    }

    public static class COSTMsgRecord extends DBRecord {
    	
    	int agent;
    	long messages;
    	String testFile;

        public COSTMsgRecord(int agent, long msgCount, String testFile) {
            this.agent = agent;
