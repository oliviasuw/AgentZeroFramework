...
@Register(name = "interMsgs")
public class InterMsgs extends AbstractStatisticCollector<InterMsgs.MultiVarMsgRecord> {
    ...
    @Override
    public void submit(MultiVarMsgRecord record) {
    ...
    public VisualModel analyze(Database db, Test r) {
    	try {
            ...
            switch (graphType) {
            case BY_AGENT:
                bv = new BarVisualModel("Inter-agent Messages Count", "Agent", "Avg(Message Sent)");
                res = db.query(""
            ...
            case BY_TESTFILE:
                ...
                bv = new BarVisualModel("Inter-agent Messages Count", runVar, "Sum(Message Sent)");
                res = db.query(""
            ...
            Logger.getLogger(InterMsgs.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void hookIn(final Agent[] agents, final Execution ex) {
        ...
        new Hooks.BeforeMessageSentHook() {
            ...
            public void hook(int sender, int recepiennt, Message msg) {
            	if(!varAgentMap.get(sender).equals(varAgentMap.get(recepiennt)))
            		counts[sender]++;
            }
        }.hookInto(ex);
        ...
        new Hooks.TerminationHook() {
            ...
            public void hook() {
                for (int i = 0; i < counts.length; i++) {
                    submit(new MultiVarMsgRecord(i, counts[i], testFile));
                }
            }
        }.hookInto(ex);
    }
    ...
    public String getName() {
    	...
    	return "Inter-Agent Messages Count";
    }

    //THIS IS YOUR RECORD IN THE DATABASE DEFINITION
    public static class MultiVarMsgRecord extends DBRecord {
    	
    	int agent;
    	long messages;
    	String testFile;

        public MultiVarMsgRecord(int agent, long msgCount, String testFile) {
            this.agent = agent;
