...
@Register(name = "FB_ESTIMATEMessageCounter")
public class FBESTIMATEMessageCounter extends AbstractStatisticCollector<FBESTIMATEMessageCounter.FBESTIMATEMsgRecord> {
    ...
    @Override
    public void submit(FBESTIMATEMsgRecord record) {
    ...
    public VisualModel analyze(Database db, Test r) {
    	try {
            ...
            switch (graphType) {
            case BY_AGENT:
                bv = new BarVisualModel("FB_ESTIMATE Messages Count", "Agent", "Avg(Message Sent)");
                res = db.query(""
            ...
            case BY_TESTFILE:
                ...
                bv = new BarVisualModel("FB_ESTIMATE Messages Count", runVar, "Sum(Message Sent)");
                res = db.query(""
            ...
            Logger.getLogger(FBESTIMATEMessageCounter.class.getName()).log(Level.SEVERE, null, ex);
        }
            ...
            case single:
                ...
                new Hooks.BeforeMessageSentHook() {
                    ...
                    public void hook(int sender, int recepiennt, Message msg) {
                        if(!varAgentMap.get(sender).equals(varAgentMap.get(recepiennt)) && msg.getName().equals("FB_ESTIMATE"))
                            counts[sender]++;
                    }
                }.hookInto(ex);
            case multiple:
                new Hooks.BeforeMessageSentHook() {
                    @Override
                    public void hook(int sender, int recepiennt, Message msg) {
                        if(msg.getName().equals("FB_ESTIMATE"))
                            counts[sender]++;
                    }
                }.hookInto(ex);
        }
        ...
        new Hooks.TerminationHook() {
            ...
            public void hook() {
                for (int i = 0; i < counts.length; i++) {
                    submit(new FBESTIMATEMsgRecord(i, counts[i], testFile));
                }
            }
        }.hookInto(ex);
    }
    ...
    public String getName() {
    	...
    	return "FB_ESTIMATE Messages Count";
    }

    //THIS IS YOUR RECORD IN THE DATABASE DEFINITION
    public static class FBESTIMATEMsgRecord extends DBRecord {
    	
    	int agent;
    	long messages;
    	String testFile;

        public FBESTIMATEMsgRecord(int agent, long msgCount, String testFile) {
            this.agent = agent;
