/**
 * AC_BnBAdoptPlusAgent_OC_External.java
   Created by Su Wen
   Date: Apr 1, 2015
   Time: 7:33:00 PM 
 */
...
@Algorithm(name="AC_BnBAdoptPlusAgent_OC_External", useIdleDetector=true)   // Corresponds to the algorithm name in the .xml file
public class AC_BnBAdoptPlusAgent_OC_External extends SimpleAgent {
    ...
    // Here, principal variables are actually external variables
    private ArrayList<Integer> identifyPrincipalVars () {
    	...
    	for (int myVar : myVars) {
    		...
    		for (int neighbor : myNeighbors) {
    			List<Integer> varsInNeighbor = getProblem().getVariables(neighbor);
    			for (int varInNeighbor : varsInNeighbor) {
    				if (getProblem().isVarConstrained(myVar, varInNeighbor)) {
    					isPrincipal = true;
